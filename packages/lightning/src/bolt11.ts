...
const MILLISATS_PER_MILLIBTC = new BigNumber(1e8, 10);
const MILLISATS_PER_MICROBTC = new BigNumber(1e5, 10);
const MILLISATS_PER_NANOBTC = new BigNumber(1e2, 10);
const PICOBTC_PER_MILLISATS = new BigNumber(10, 10);
...
// TODO: this is copied from packages/utxo-lib/src/address.ts
...
function humanReadablePartToMillisat (humanReadableValue: string, multiplierLetter: MultiplierLetter) {
    ...
    // if (humanReadableValue.slice(-1).match(/^[munp]$/)) {
    // //   divisor = hrpString.slice(-1) as any; // TODO: fix this!!!
    //   value = humanReadableValue.slice(0, -1)
    // } else if (humanReadableValue.slice(-1).match(/^[^munp0-9]$/)) {
...
export const decodePaymentRequest = (paymentRequest: string, network?: string) => {
    ...
    return finalResult; // TODOL order it!!!
};
